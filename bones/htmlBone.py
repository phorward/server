...
class htmlBone(baseBone):
	...
	def __init__(self, validHtml=__undefinedC__, indexed=False, languages=None, maxLength=200000, *args, **kwargs):
		...
		if indexed:
			raise NotImplementedError("indexed=True is not supported on htmlBones")
		if self.multiple:
			raise NotImplementedError("multiple=True is not supported on htmlBones")
		if validHtml == htmlBone.__undefinedC__:
			global _defaultTags
